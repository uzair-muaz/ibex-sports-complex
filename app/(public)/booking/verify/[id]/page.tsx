...
export default function VerifyBookingPage() {
  ...
  const startTimeFormatted = `${Math.floor(booking.startTime)
    .toString()
    .padStart(2, "0")}:${booking.startTime % 1 === 0 ? "00" : "30"}`;
  const rawEndTime = booking.startTime + booking.duration;
  const endTime = ((rawEndTime % 24) + 24) % 24;
  const endTimeFormatted = `${Math.floor(endTime)
    .toString()
    .padStart(2, "0")}:${endTime % 1 === 0 ? "00" : "30"}`;
            ...
            {(booking.amountPaid || 0) > 0 && (
              <div className="pt-4 border-t border-zinc-800">
                <div className="flex items-center justify-between">
                  ...
                  <p className="text-white font-semibold">
                    PKR {(booking.amountPaid || 0).toLocaleString()}
                  </p>
                </div>
